...
int main()
{
	...
	if (ranka == 1)
	{
		...
		for (int i = 0; i < 5; i++)
		{
			...
			start = clock();
			SRSmegNemk(Mok, Smegenines, Nemoksos);
			end = clock();
			cout << "Failo su " << Smegenines.size() + Mok.size() << " irasu dalijimo i dvi dalis laikas, panaikinant pradini vektoriu: "
			...
			isvestis(Smegenines, Mok, ciklas, kiek, ofsGeri, ofsBlogi);		//f-ja skirta isvedimui
			ciklas = 0;
			Smegenines.clear();
			Nemoksos.clear();
			ofsGeri.close();
